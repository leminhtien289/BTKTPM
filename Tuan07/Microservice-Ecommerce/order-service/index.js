...
// ── POST /orders — full saga: validate → inventory → create → payment → ship ──
app.post('/orders', async (req, res) => {
  ...
  try {
    // Step 1: validate customer + product in parallel
    ...
    // Step 2: reduce inventory (Inventory Service is source of truth for stock)
    await axios.put(`${INVENTORY_SERVICE}/inventory/${product_id}/reduce`,
      { quantity }, { timeout: 3000 });

    // Step 3: persist order as 'pending'
    const total_price = product.price * quantity;
    const r = db.prepare(`
      INSERT INTO orders
        (customer_id, customer_name, product_id, product_name, quantity, unit_price, total_price, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')
    ...
    try {
      ...
      if (payment.status === 'completed') {
        ...
        // Step 5: create shipment (non-blocking — failure doesn't cancel order)
        try {
          ...
          console.warn(`[order ${orderId}] Shipment creation failed: ${shipErr.message}`);
        }
      } else {
        // Payment declined → rollback inventory
        finalStatus = 'payment_failed';
        await axios.put(`${INVENTORY_SERVICE}/inventory/${product_id}/restore`,
          { quantity }).catch(() => {});
      }
    } catch (payErr) {
      // Payment service unreachable → rollback inventory
      finalStatus = 'payment_error';
      await axios.put(`${INVENTORY_SERVICE}/inventory/${product_id}/restore`,
        { quantity }).catch(() => {});
      console.error(`[order ${orderId}] Payment error: ${payErr.message}`);
    }
...
app.listen(PORT, () => console.log(`Order Service (B7) on port ${PORT}`));
